...
function closeRoomIfEmpty(rooms, io, room) {
  if(room.isEmpty()) {
    rooms = _.without(rooms, room);
    io.sockets.emit('roomclosed', room.name);
  }
}
  ...
  onLeave: function(socket, name, ack) {
    if(this.registery.isIdentified(socket.id)) {
      ...
      if(room !== undefined) {
        ...
        closeRoomIfEmpty(this.rooms, this.io, room);
      } else {
    ...
    if(this.registery.isIdentified(socket.id)) {
      ...
      socket.rooms.forEach(function(roomName) {
        ...
        if(room !== undefined) {
          ...
          closeRoomIfEmpty(_this.rooms, _this.io, room);
        }
      });
    }
  }
